Feature: Harding+ Careers Page
  ...
  Scenario Outline: Job Application
    ...
    Then I should be able to fill in the '<mandatory fields>' with the Basic data and upload '<file>' on the first page
      ...
      | New  | nameclaim@yahoo.com | 07123123355  | York |
    And I should get the '<expected result>' when submitting the application.

    Examples:
      | mandatory fields | file                                                             | expected result        |
      | Name             | C:\Automation\Harding+\Harding\src\main\resources\PDF upload.exe |                        |
      | Fame            | C:\Automation\Harding+\Harding\src\main\resources\png upload.exe | This field is required |
      | Claim              | C:\Automation\Harding+\Harding\src\main\resources\exeupload.exe  | This field is required |
